...
if not os.path.exists(archivo_excel):
    st.error(f"❌ No se encontró el archivo '{archivo_excel}' en la carpeta. Por favor créalo para cargar los grupos.")
    st.stop()
...
# 3. SISTEMA DE GUARDADO PERMANENTE (JSON)
ARCHIVO_PRONOSTICOS = "pronosticos.json"

def cargar_pronosticos():
    if os.path.exists(ARCHIVO_PRONOSTICOS):
        with open(ARCHIVO_PRONOSTICOS, "r", encoding="utf-8") as f:
            try:
                datos = json.load(f)
                datos_limpios = {}
...
def guardar_pronosticos(datos):
    with open(ARCHIVO_PRONOSTICOS, "w", encoding="utf-8") as f:
        json.dump(datos, f, indent=4)

# 4. CONTROL DE BASE DE DATOS LOCAL
...
with tab1:
    ...
    # IDENTIFICACIÓN FUERA DEL FORMULARIO (La clave para poder consultar)
    ...
    # SOLO MOSTRAR LOS PARTIDOS SI EL USUARIO YA SE IDENTIFICÓ
    if nombre_input:
        ...
        # Recuperar pronósticos del usuario (si es nuevo, esto traerá un diccionario vacío)
        ...
        with st.form("form_pronosticos"):
            ...
            for id_partido, info in PARTIDOS.items():
                ...
                if hora_actual >= hora_limite_cierre:
                    ...
                    deshabilitado = False
                    estado_texto = f"🟢 (Abierto - Cierra el {hora_limite_cierre.strftime('%d/%m %H:%M')})"
                
                st.markdown(f"**Partido {id_partido}: {info['local']} vs {info['visita']}** | {estado_texto}")
                col1, col2 = st.columns(2)
                
                # LA MAGIA ESTÁ AQUÍ: Cargar el valor que guardó ayer, o poner 0 si es partido nuevo
        ...
        if boton_enviar:
            ...
            st.success(f"¡Pronósticos de {nombre_input} actualizados y guardados con éxito!")
    else:
...
with tab3:
    ...
    st.write("Esta es la lista de partidos cargada desde tu documento `partidos.xlsx`:")
